...
- `grep` searches the repository, and `read_lines` reads a span of one file.
- `list_docs` names the repository's own design notes, decision records and
  READMEs. It is the check that keeps a finding off a construction the team
  wrote down on purpose, which is where the dismissed findings come from.
- `symbol_context` asks gorefactor for one Go symbol's definition, its callers
  resolved through the type checker, its signature types and its tests. The
  callers are facts rather than name matches. It needs `gorefactor` on PATH and
  is left off the catalogue where it is missing, because a tool the reviewer can
  see and cannot use costs it a turn to find that out.
- `line_history` asks git why a span of lines is there: the commits that last
...
does, the composition table, then a collapsible walkthrough of every changed
file, test files aside, with how many lines that file moved and the agent's
one-line summary or "No notes." A finding that names one of those files rides
under it there; only a finding with no file lands in the list after. `body_include` decides how much of the report rides along:
