...
| Item | What | Effort |
|---|---|---|
| **Let the review look things up itself** | Started, and the part that shipped is the cheap part. `--look` gives the judging passes `grep` and `read_lines`: they answer in the conversation, record nothing, and cost about 600 input tokens on the catalogue because it rides the cached prefix. That is two of the six tools the scout has. What is left is the rest of the shape - the reviewer asking gorefactor for callers resolved through a type checker, and the graph for a cross-language path - and the decision that follows: a reviewer that checks its own claims is a reviewer `internal/scout` and the ruling exist to serve, and there is then nothing left for a separate ruling to reconcile. Explore mode is not this: its `fetch_context` only indexes what the packet already resolved and cannot search the tree, so that loop buys a shorter prompt rather than more thought. Three runs on one real pull request have now measured it, same revision and model: plain 3 turns and $0.23; `--look --defer-context` 9 turns and $0.42, which read the source view and correctly retired a warning the plain run filed; `--look` with context inline 10 turns and $1.09, which found three things neither other run did, all of them drift between the diff and files outside it, including a UI footnote telling users a distortion cannot happen when the SQL still falls back to the case that causes it. So searching does find what the current shape cannot, and what it finds is cross-artifact inconsistency rather than the logic defects the gap above is chasing. One pull request cannot rank the three shapes, and the spread from $0.23 to $1.09 is the open question: deferring context is what makes searching cheap, and it is also the run that found the least. Hold the cost estimate loosely: Alibaba's `ocr` is this shape run all the way out, and on a 43-file change it went past $20 a review, blew through its own token cap by 150% to 235%, and posted two confident false positives while finding one real bug nothing else found. | L |
...
| **Context lines on grep** | `grepTree` returns one trimmed line per match and there is no `-C`. So the reviewer greps, cannot tell which of sixty matches is worth opening, and spends a `read_lines` turn to find out - the traces show it doing exactly that, three times in a row on one question. Turns are what the $1.09 is made of, so this is the cheapest thing that could reduce it: the line loop already holds the file, so it is a `context` parameter and two bounds. The cap has to move with it, from sixty matches to a ceiling on total lines, or one wide pattern returns three hundred. | S |
| **`list_docs` in the findings pass** | The third run found the stale sibling doc by guessing a path. `list_docs` is the inventory that makes it a lookup rather than luck: every document in the tree with its first heading, one call, no arguments. It is also the only tool that answers a `rule`-kind question - did the team write this down - which currently has none on the judging side. A change that moves a metric definition almost always leaves some document behind, and grep answers that badly because you have to guess the word the doc used. | S |
| **`gorefactor_context` in the findings pass** | The one remaining scout tool that strictly dominates what `--look` already has: callers resolved by type identity rather than matched as text, so they are facts and not leads. It is what would have given the reviewer a chance at the kind of finding that needs two files at once - the cohort-context filter bug in #81 needed the envelope contract and both providers' emission code, which no single file shows. Needs the binary and a Go tree, so it has to degrade to a note where either is missing. The graph tools stay on the scout: they need a build artifact most repositories lack, and they match by name, so their answers are leads and a findings pass would be filing claims grounded in a guess. | M |
