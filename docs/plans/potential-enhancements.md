...
| **The answerer passes no model, effort, or cost cap** | `cmd/redline/review.go` builds `scout.Options` with root, diff, questions and credentials only. `--model` and `--effort` cannot reach the lookups, and `MaxCostUSD` is zero, so the scout's own governor is off for the one path that runs it several times per review. | S |
