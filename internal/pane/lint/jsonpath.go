...
// globToRegexp compiles a scope glob to an anchored regexp. Recompiled per
// call rather than cached, because the pattern set is tiny and a package-level
// cache would need guarding against the test suite's parallel runs.
func globToRegexp(pattern string) *regexp.Regexp {
	...
	for i := 0; i < len(pattern); {
		...
		case strings.HasPrefix(pattern[i:], "**/"):
			// A leading "**/" matches any number of directory segments,
			// including none, so "**/*.ts" covers a file at the root too.
			b.WriteString("(?:.*/)?")
			i += 3
		case strings.HasPrefix(pattern[i:], "**"):
			b.WriteString(".*")
