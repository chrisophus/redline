...
// actually sent.
func toolsTokens(describes, recaps, pulls bool, looks []string) int {
	raw, err := json.Marshal(callTools(describes, recaps, pulls, looks))
	if err != nil {
...
func anthropicTools(describes, recaps, pulls bool, looks []string) []anthropic.ToolUnionParam {
	all := callTools(describes, recaps, pulls, looks)
...
func openAITools(describes, recaps, pulls bool, looks []string) []openAITool {
	all := callTools(describes, recaps, pulls, looks)
