...
// actually sent.
func toolsTokens(pulls bool) int {
	raw, err := json.Marshal(callTools(pulls))
	if err != nil {
...
func anthropicTools(pulls bool) []anthropic.ToolUnionParam {
	all := callTools(pulls)
...
func openAITools(pulls bool) []openAITool {
	all := callTools(pulls)
